...
TEST_CASE("Test irregular polygon alignment") {
    ...
    SUBCASE("Irregular trapezoid polygon") {
        ...
        concord::Polygon trapezoid;
        trapezoid.addPoint({10, 20, 0});   // Bottom-left (offset from origin)
        trapezoid.addPoint({90, 15, 0});   // Bottom-right (slightly lower)
        trapezoid.addPoint({85, 85, 0});   // Top-right (not square)
        trapezoid.addPoint({15, 80, 0});   // Top-left (not square)
        ...
        if (grid_data.hasGrids()) {
            ...
            std::cout << "Grid center (shift): " << shift.point.x << ", " << shift.point.y << ", " << shift.point.z << std::endl;
    ...
    SUBCASE("Diamond-shaped polygon") {
        ...
        concord::Polygon diamond;
        diamond.addPoint({50, 0, 0});   // Bottom
        diamond.addPoint({100, 50, 0}); // Right
        diamond.addPoint({50, 100, 0}); // Top
