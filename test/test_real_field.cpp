...
TEST_CASE("Test real irregular field from /home/bresilla/vector.geojson") {
    
    SUBCASE("Real field polygon analysis") {
        // Use geoson library to read the GeoJSON file
        auto feature_collection = geoson::ReadFeatureCollection("/home/bresilla/vector.geojson");
        ...
        if (grid_data.hasGrids()) {
            ...
            std::cout << "  Min: " << std::fixed << std::setprecision(6) << aabb.min_point.x << ", " << aabb.min_point.y << std::endl;
            std::cout << "  Max: " << std::fixed << std::setprecision(6) << aabb.max_point.x << ", " << aabb.max_point.y << std::endl;
