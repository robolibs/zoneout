...
inline Timestamp fromMilliseconds(uint64_t ms) {
...
inline bool isFuture(const Timestamp& timestamp) {
    return timestamp > now();
}

// Check if timestamp is in the past
inline bool isPast(const Timestamp& timestamp) {
    return timestamp < now();
}

// Check if duration has elapsed since timestamp
inline bool hasElapsed(const Timestamp& start, const Duration& duration) {
...
inline Timestamp add(const Timestamp& timestamp, const Duration& duration) {
    return timestamp + duration;
}

// Subtract duration from timestamp
inline Timestamp subtract(const Timestamp& timestamp, const Duration& duration) {
    return timestamp - duration;
}
...
inline Duration estimateClockOffset(const Timestamp& local_send_time,
                                   const Timestamp& remote_time,
